...
def part_needs_exclusion(part, exclusion_keywords, previous_part=None):
    for keyword in exclusion_keywords:
        if keyword in part.split():  # Разделяем часть адреса на слова и проверяем точное совпадение
            # Не исключаем часть адреса, если перед ней идет "мкр." или другое слово из списка исключений из исключений
            if previous_part and 'мкр.' in previous_part.split():
                return False
            return True
    return False
    ...
    for raw_address in raw_data:
        ...
        for level, part in enumerate(parts, start=1):
            if part == country_name:
                continue
            if part_needs_exclusion(part, exclusion_keywords, previous_part):
